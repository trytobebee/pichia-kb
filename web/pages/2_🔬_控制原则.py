...
with tab5:
    proto_data = kb.get_process_knowledge("fermentation_protocols")
    if not proto_data:
        st.info("暂无数据")
    else:
        st.caption(f"共 {len(proto_data)} 份完整工艺单(从 7 篇论文综合而来)")
        for proto in proto_data:
            name = proto.get("name", "未命名协议")
            yield_str = proto.get("expected_yield") or ""
            head = f"📋 **{name}**" + (f"  —  产量 {yield_str}" if yield_str else "")
            with st.expander(head):
                # Top: who/what/how-long
                c1, c2 = st.columns(2)
                with c1:
                    if proto.get("target_product"):
                        st.markdown(f"**目标产物：** {proto['target_product']}")
                    if proto.get("host_strain"):
                        st.markdown(f"**宿主菌株：** {proto['host_strain']}")
                with c2:
                    if proto.get("expression_vector"):
                        st.markdown(f"**表达载体：** {proto['expression_vector']}")
                    dur = proto.get("total_duration_hours")
                    if dur is not None:
                        st.markdown(f"**总时长：** {dur} h")

                if proto.get("expected_yield"):
                    st.success(f"**预期产量：** {proto['expected_yield']}")

                csf = proto.get("critical_success_factors") or []
                if csf:
                    st.markdown("**关键成功因素：**")
                    for f in csf:
                        st.markdown(f"  - {f}")

                qc = proto.get("quality_checkpoints") or []
                if qc:
                    st.markdown("**质量检验点：**")
                    for c in qc:
                        st.markdown(f"  - {c}")

                if proto.get("_source_doc"):
                    st.caption(f"来源：{proto['_source_doc']}")
